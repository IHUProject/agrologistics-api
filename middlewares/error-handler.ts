  ...
  if (err.name === 'ValidationError') {
    const errors = Object.values(err.errors).map(item => {
      ...
      if (item.kind === 'minlength') {
        return `The ${item.path} must be more than ${item.properties.minlength - 1} characters.`
      }
      if (item.kind === 'maxlength') {
        return `The ${item.path} must be less than ${item.properties.maxlength + 1} characters.`
      }
      return item
    })
